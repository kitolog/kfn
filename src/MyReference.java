...
public class MyReference implements PsiReference {
    ...
    public MyReference(String path, PsiElement element, TextRange textRange, Project project, ArrayList<VirtualFile> resultDirs, String className, Boolean kohanaPSR) {
    ...
    public PsiElement resolve() {
        ...
        } else if (MyPsiReferenceProvider.checkClass("request")) {
            String[] parts = path.split("/");
            if (parts.length > 1) {
                action = parts[1];
            }
            path = parts[0].toLowerCase().replace('_', '/');
        } else if (MyPsiReferenceProvider.checkClass("config")) {
            String[] parts = path.split("\\.");
            path = parts[0].toLowerCase();
        } else if (!MyPsiReferenceProvider.checkClass("view")) {
            path = path.toLowerCase().replace('_', '/');
        }
